...
| **Execution Backends** | Run real actions: SSH executor (Phase 7, DEC-037 to DEC-042) and the GitHub connector (Phase 11, DEC-058 to DEC-063) are implemented; the pattern is open to more connectors. Build commands/operations in a parameterized way, never by interpolating LLM-generated strings. | DECISION + IMPLEMENTED (see §9) |
...
**DECISION (DEC-058 to DEC-063, Phase 11 — Connectors):** the "API connector (future)" mentioned
as PROPOSAL under the **Execution Backends** component (§2) is now decided and materialized with a
first real connector, GitHub — see `decisions/DECISIONS.md`. Summary: **same pattern as SSH
Execution** — its own package (`packages/connector-github`), an independent Execution process,
never a child of the MCP server (DEC-058); reuses `ExecutionRequest`/`ExecutionOutcome`/
`ExecutionChannelRequest` with no new protocol, with `hostId` reinterpreted as the configured
account identifier (already opaque in its type) — the MCP server routes between Execution Backend
processes by `ToolEntry.origin.id`, still a single MCP server (DEC-043/059); a new additive
variant `ExecutionOutcome.kind === "executed-http"` with `statusCode`/`responseBytes` — never
forcing an HTTP response into `"executed"`'s SSH-shaped fields, the response body is never
recorded (DEC-060); authentication via a Personal Access Token through the already-existing
`SecretKind "token"`, no OAuth in this phase (DEC-061); 3 GitHub operations with a fixed
endpoint+method+payload template — never a free-form HTTP method/path/body from the agent, same
principle as DEC-037 (DEC-062); Node's native `fetch`, no new HTTP dependency (DEC-063). **Known
limitation, shared with SSH Execution:** neither Execution Backend has a real channel to the
Secrets Broker in production today — both receive the secret through an injected function with no
real implementation (the Core↔Secrets Broker channel from DEC-010 still has no implementation on
any operating system); out of scope for this phase, a candidate for Phase 13 (Hardening) or a
dedicated phase.
...
**DECISION (DEC-043 to DEC-047, Phase 8):** the MCP server's count and location, human
confirmation during `tools/call`, transport, and process separation are already decided — see
`decisions/DECISIONS.md`. Summary: a **single MCP server** (`packages/mcp-server`), agnostic of
the execution backend, consuming the already-built Discovery/Policy Engine/Execution without
reimplementing them; **stdio** transport with Claude Code, never writing non-MCP content to
stdout; human confirmation during `tools/call` via periodic progress
(`notifications/progress`) and explicit `notifications/cancelled` handling, extending DEC-038
without modifying it; and, crucially, **the MCP server and the Execution process run as separate
processes** (DEC-047) — the operator starts Execution independently, connected to the MCP server
over a channel following DEC-010's transport pattern (a new domain interface, without modifying
`SecretsBrokerTransport`) — precisely because stdio occupies the MCP server's stdin/stdout, in
direct conflict with `ReadlineConfirmationChannel` (DEC-038). This process-separation design
resolves §1's open sequencing question: MCP as the primary discovery/invocation mechanism in this
phase; Claude Code hooks remain a documented, possible future cross-cutting extension, not
implemented.

**PROPOSAL (historical context, Phase 1 — see DEC-043 to DEC-047 above for what is now decided):**
...
**Phase 8 (DEC-043 to DEC-047) resolves the remaining implementation questions.**

---

## 11. Sessions and state

**DECISION (DEC-048 to DEC-051, Phase 9):** Sessions' scope, model, origin, and location are
already decided — see `decisions/DECISIONS.md`. Summary: **single-user/single-agent** scope in
this phase, without reopening DEC-047; `SessionId` as a **lightweight correlation identifier**,
never a `Session` entity that owns or merges the already-existing registries of the Policy Engine
(`InMemoryPolicyApprovalStore`) or Execution (`OperationHashRegistry`) — those registries remain
exactly as they are; generated by the MCP server itself at startup (not derived from the MCP SDK:
technical verification confirmed `StdioServerTransport`, DEC-046, never exposes a transport-level
`sessionId` — that capability is exclusive to HTTP/Streamable transports with reconnection); type
lives in `packages/shared`, no dedicated package or process.

**PROPOSAL (historical context, Phase 1 — see DEC-048 to DEC-051 above for what is now decided):**
...
some point~~ — **resolved in Phase 9, see DEC-048**: not in this phase; multi-agent/multi-user
...
- The separated-component architecture (Tool Registry, Policy Engine, Secrets Broker, Execution
  ...
  Zone/Platform Zone in Arcade). **Confirmed in practice in Phase 11** (DEC-058 to DEC-063, see
  §9): the GitHub connector was added as `packages/connector-github` without touching
  `execution-ssh`, the Policy Engine, or the Secrets Broker.
...
1. ~~Exact sequencing: start with own MCP and add cross-cutting hooks later, or both from the
   start? (§1)~~ — **resolved in Phase 8, see DEC-043 to DEC-047**: own MCP as the primary
   mechanism; hooks remain a possible future extension, not implemented.
